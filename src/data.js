...
// construtoras/condomínios/Blow (JSON grandes) saíram p/ ./empreendimentos.js — só páginas lazy os usam
...
// Construtoras, empreendimentos (construtoras.json + Blow) e condomínios moveram-se
// para ./empreendimentos.js (JSON grandes, só páginas lazy) — fora do bundle principal.
...
export const destaquesImovel = (im) => {
  ...
  if (im.suites > 0) d.push({ icon: 'sparkle', titulo: `${im.suites} ${_plural(im.suites, 'suíte', 'suítes')}`, sub: 'Com banheiro privativo' })
  if (im.quartos > im.suites) d.push({ icon: 'bed', titulo: `${im.quartos} ${_plural(im.quartos, 'quarto', 'quartos')}`, sub: 'Espaço para todos' })
  if (im.area > 0) d.push({ icon: 'area', titulo: formatArea(im.area), sub: 'Área privativa' })
  if (im.vagas > 0) d.push({ icon: 'car', titulo: `${im.vagas} ${_plural(im.vagas, 'vaga', 'vagas')} na garagem`, sub: 'Espaço coberto para seu carro' })
  ...
  if (im.andar > 0) d.push({ icon: 'floor', titulo: `${im.andar}º andar`, sub: 'Vista privilegiada' })
  // diferenciais reais (características / descrição)
  if (tem(/piscina/)) d.push({ icon: 'pool', titulo: 'Piscina', sub: 'Lazer e bem-estar em casa' })
  if (tem(/varanda gourmet/)) d.push({ icon: 'balcony', titulo: 'Varanda gourmet', sub: 'Perfeito para receber' })
  if (tem(/churrasqueira/)) d.push({ icon: 'grill', titulo: 'Churrasqueira', sub: 'Encontros com a família' })
  if (tem(/fotovolta|energia solar|aquec.*solar/)) d.push({ icon: 'solar', titulo: 'Energia solar', sub: 'Economia e sustentabilidade' })
  if (tem(/closet/)) d.push({ icon: 'closet', titulo: 'Closet', sub: 'Organização e sofisticação' })
  if (tem(/elevador/)) d.push({ icon: 'elevator', titulo: 'Elevador', sub: 'Acesso prático ao seu andar' })
  ...
  if (tem(/academia/)) d.push({ icon: 'gym', titulo: 'Academia', sub: 'Saúde sem sair de casa' })
  if (tem(/salão de festas|salao de festas/)) d.push({ icon: 'party', titulo: 'Salão de festas', sub: 'Espaço para comemorar' })
  if (tem(/home.?office|escritório/)) d.push({ icon: 'target', titulo: 'Home office', sub: 'Ambiente para trabalhar em casa' })
  if (tem(/jardim/)) d.push({ icon: 'home', titulo: 'Jardim', sub: 'Área verde privativa' })
  if (tem(/spa|ofurô|ofuro/)) d.push({ icon: 'bath', titulo: 'Spa / ofurô', sub: 'Relaxamento em casa' })
  if (tem(/coworking/)) d.push({ icon: 'target', titulo: 'Coworking', sub: 'Espaço compartilhado para trabalhar' })
  if (tem(/playground/)) d.push({ icon: 'sparkle', titulo: 'Playground', sub: 'Área de lazer para as crianças' })
  if (tem(/pet.?place|espa.o.?pet/)) d.push({ icon: 'home', titulo: 'Pet place', sub: 'Espaço dedicado ao seu pet' })
